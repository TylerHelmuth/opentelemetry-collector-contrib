...
func (p *Parser[K]) evaluateMathValue(val *mathValue) (Getter[K], error) {
	switch {
	case val.ConstValue != nil:
		return p.newGetter(value{
			Invocation: val.ConstValue.Invocation,
			Path:       val.ConstValue.Path,
			Int:        val.ConstValue.Int,
			Float:      val.ConstValue.Float,
		})
	case val.SubEquation != nil:
		return p.evaluateMathExpression(val.SubEquation)
	}
