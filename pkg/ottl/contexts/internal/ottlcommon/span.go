...
func SpanPathGetSetter[K SpanContext](path ottl.Path) (ottl.GetSetter[K], error) {
	...
	case "trace_state":
		...
		return accessTraceStateKey[K](path.Keys), nil
	case "parent_span_id":
		if len(path.Fields) == 1 {
	...
	case "attributes":
		...
		return accessAttributesKey[K](path.Keys), nil
...
func accessTraceStateKey[K SpanContext](keys []ottl.Key) ottl.StandardGetSetter[K] {
	return ottl.StandardGetSetter[K]{
		Getter: func(ctx context.Context, tCtx K) (interface{}, error) {
		...
		Setter: func(ctx context.Context, tCtx K, val interface{}) error {
			if str, ok := val.(string); ok {
				if ts, err := trace.ParseTraceState(tCtx.GetSpan().TraceState().AsRaw()); err == nil {
					if updated, err := ts.Insert("", str); err == nil {
