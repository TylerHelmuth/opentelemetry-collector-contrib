...
type convertSummaryCountValToSumFactory struct{}

func (f convertSummaryCountValToSumFactory) GetFunctionType() reflect.Type {
	return reflect.TypeOf(f.convertSummaryCountValToSum)
}

func (f convertSummaryCountValToSumFactory) CreateFunction(args []reflect.Value) (ottl.ExprFunc[ottldatapoint.TransformContext], error) {
	return f.convertSummaryCountValToSum(args[0].String(), args[1].Bool())
}

func (f convertSummaryCountValToSumFactory) convertSummaryCountValToSum(stringAggTemp string, monotonic bool) (ottl.ExprFunc[ottldatapoint.TransformContext], error) {
